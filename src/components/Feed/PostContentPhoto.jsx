...
const PostContentPhoto = ({ data }) => {
  ...
  const AddPhoto = async (e) => {
    ...
    try {
      const imgRef = ref(storage, `Photo/${data.email}/${profileDetails.image.name}`);
      await uploadBytes(imgRef, profileDetails.image);
        ...
        <Card className="cardContainer mx-auto">
          <form onSubmit={AddPhoto}>
            <CardHeader className="space-y-1">
              <CardTitle className="text-2xl font-bold">Add Your Photo</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <Label htmlFor="backgroundImage">Photo</Label>
                  <Input
                    id="backgroundImage"
                    type="file"
                    accept="image/*"
